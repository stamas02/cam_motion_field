...
def get_cam_flow(image1, image2, n_rows, n_cols):
    ...
    for y0, y1 in zip(row_pixel_indexes, row_pixel_indexes[1::]):
        for x0, x1 in zip(col_pixel_indexes, col_pixel_indexes[1::]):
            ...
            idx = np.where((origins[:, 0] > x0) & (origins[:, 0] <= x1) & (origins[:, 1] > y0) & (origins[:, 1] <= y1))[0]
            if len(idx) == 0: # if no origin was found assign a zero displacement
                block_dispalcement = np.array([cx, cy])
            else:
                block_dispalcement = np.mean((displacements[idx]-origins[idx])+np.array([cx, cy]), axis=0, dtype=np.int)
